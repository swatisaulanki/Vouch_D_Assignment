...
function ModalData({ showdatas, handleClosData, token, Ton }) {
      ...
      <div className="Modal">
        <div className="massege-box">
          ...
          <div className="log_button">
            {showdatas ? (
              <NavLink to="/Datav_Clients" className="ok-btn">
                <button className="btn btn-primary " onClick={storingData}>
                  Ok
                </button>
              </NavLink>
            ) : (
...
const Login = () => {
  ...
  const handleClosData = () => setShow(false);
  ...
  const submitData = async () => {
    ...
    if (userData.token !== "" && DataList.status === 200) {
      ...
      setTon("Login Successfully");
      handleShow();
      setShowdatas(true);
    } else if (DataList.status === 400) {
      setTon("Page Not Found Try Again");
      handleShow();
      setShowdatas(false);
    } else if (!userData.token) {
      setTon("User Not Exist, Try Again");
      handleShow();
      setShowdatas(false);
    }
  };
